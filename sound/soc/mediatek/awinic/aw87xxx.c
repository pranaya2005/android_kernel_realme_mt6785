...
#define AW87XXX_DRIVER_VERSION	"V0.1.12"
...
static void aw87xxx_active_flag_start(struct aw87xxx *aw87xxx);
static void aw87xxx_active_flag_stop(struct aw87xxx *aw87xxx);
...
static int aw87xxx_audio_off(struct aw87xxx *aw87xxx)
{
	...
	aw87xxx_active_flag_stop(aw87xxx);
				  ...
				  struct aw87xxx_scene_param *scene_param)
{
	...
	if (scene_param->cfg_update_flag == AW87XXX_CFG_OK) {
		...
		for (i = 0; i < load_cnt->len; i = i + 2) {
			...
			if (load_cnt->data[i] == aw87xxx->gain_reg_addr &&
					aw87xxx->denoise_flag) {
				aw87xxx_set_gain(aw87xxx,
					aw87xxx->gain_hold_flag);
				continue;
			}
	...
	aw87xxx_active_flag_start(aw87xxx);
				       ...
				       const struct firmware *cont)
{
	...
	for (i = 0; i < scene_cnt->len; i = i + 2) {
		aw_dev_info(aw87xxx->dev, "%s: addr:0x%02x, data:0x%02x\n",
			    ...
			    scene_cnt->data[i + 1]);
	if (scene_cnt->data[i] == aw87xxx->gain_reg_addr) {
			aw87xxx->raw_gain_array[current_scene->scene_mode] =
			    scene_cnt->data[i + 1];
			aw_dev_info(aw87xxx->dev,
				    "%s: %s_i2c%d@0x%02X mode:%d, raw_gain:0x%02x\n",
...
static int aw87xxx_set_gain(struct aw87xxx *aw87xxx, bool is_gain_hold)
{
	...
	aw_dev_info(aw87xxx->dev, "%s: gain_hold_flag = %d\n",
		__func__, aw87xxx->gain_hold_flag);
...
static int aw87xxx_actflag_thread_run(void *pdata)
{
	...
	aw_dev_info(aw87xxx->dev, "%s enter\n", __func__);
	msleep_interruptible(WAIT_DSP_OPEN_TIME);

	while (!kthread_should_stop()) {
		mutex_lock(&aw87xxx->lock);
		ret = aw_get_active_flag(active_flag, aw87xxx->pa_channel);
		if (ret < 0) {
			pr_err("%s: failed to read active_flag, ret: %d\n",
				__func__, ret);
			err_read_flag = 1;
		} else {
			err_read_flag = 0;
		...
		if (!err_read_flag && (aw87xxx->pa_channel < 2)) {
			aw_dev_info(aw87xxx->dev,
				"%s: %s_i2c%d@0x%02X, channel:%d, mode:%d raw_gain:0x%02X, actgain:0x%02X\n",
				__func__, aw87xxx_support_product[aw87xxx->product],
				aw87xxx->i2c_seq, aw87xxx->i2c_addr,
				aw87xxx->pa_channel,
			...
			if ((active_flag[aw87xxx->pa_channel] == AW_ACTIVE_ENABLE) &&
				(active_flag[1 - aw87xxx->pa_channel] != AW_ACTIVE_DISABLE))
	...
	if (flag) {
		...
		if (!aw87xxx->actflag_sync_task) {
			aw87xxx->actflag_sync_task =
				kthread_run(aw87xxx_actflag_thread_run,
						(void *)aw87xxx,
						"%s", AW87XXX_I2C_NAME);
...
static void aw87xxx_nodsp_denoise_func(struct aw87xxx *aw87xxx)
{
	aw_dev_info(aw87xxx->dev,
		"%s: %s_i2c%d@0x%02X left_flag: %d, right_flag: %d\n",
		__func__, aw87xxx_support_product[aw87xxx->product],
		aw87xxx->i2c_seq, aw87xxx->i2c_addr,
		aw87xxx->active_flag_val[0], aw87xxx->active_flag_val[1]);

	if (aw87xxx->pa_channel < 2) {
		if ((aw87xxx->active_flag_val[aw87xxx->pa_channel] == AW_ACTIVE_ENABLE) &&
			(aw87xxx->active_flag_val[1 - aw87xxx->pa_channel] != AW_ACTIVE_DISABLE)) {
			aw87xxx_set_gain(aw87xxx, true);
		} else {
			aw87xxx_set_gain(aw87xxx, false);
		}
	} else {
		aw_dev_info(aw87xxx->dev, "%s: pa_channel:%d unsupported\n",
...
static void aw87xxx_active_flag_start(struct aw87xxx *aw87xxx)
{
	if ((aw87xxx->current_mode != AW87XXX_OFF_MODE) &&
		aw87xxx->open_dsp_en && aw87xxx->actflag_auto_run &&
		aw87xxx->denoise_flag)
...
static ssize_t aw87xxx_set_actflag(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	int32_t get_actflag[2] = {0};
	struct aw87xxx *aw87xxx = dev_get_drvdata(dev);

	aw_dev_info(aw87xxx->dev, "%s enter\n", __func__);

	if (sscanf(buf, "%d %d", &get_actflag[0], &get_actflag[1]) == 2) {
		aw_dev_info(aw87xxx->dev, "%s rcv actflag[0]=%d,actflag[1]=%d\n",
			__func__, get_actflag[0], get_actflag[1]);

		mutex_lock(&aw87xxx->lock);
		aw87xxx->active_flag_val[0] = get_actflag[0];
		aw87xxx->active_flag_val[1] = get_actflag[1];
		if (!aw87xxx->open_dsp_en && aw87xxx->denoise_flag)
			aw87xxx_nodsp_denoise_func(aw87xxx);

		mutex_unlock(&aw87xxx->lock);
	} else {
		aw_dev_err(aw87xxx->dev, "%s get actflag failed\n",
			__func__);
	}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	aw_dev_info(aw87xxx->dev, "%s: get hal msg\n", __func__);
	if (!aw87xxx->open_dsp_en) {
		active_flag[0] = aw87xxx->active_flag_val[0];
		active_flag[1] = aw87xxx->active_flag_val[1];
	} else {
		ret = aw_get_active_flag(active_flag, aw87xxx->pa_channel);
		if (ret < 0) {
			aw_dev_info(aw87xxx->dev, "%s: get active flag failed\n",
					__func__);
			return ret;
		}
	}
	...
	aw_dev_info(aw87xxx->dev,
		    ...
		    __func__, aw87xxx_support_product[aw87xxx->product],
		    aw87xxx->i2c_seq, aw87xxx->i2c_addr,
		    active_flag[0], active_flag[1]);
	len += snprintf(buf + len, PAGE_SIZE - len,
		     "%s: left_flag: %d, right_flag: %d\n",
		     aw87xxx_support_product[aw87xxx->product], active_flag[0],
		     active_flag[1]);
	return len;
}
				  ...
				  const char *buf, size_t len)
{
	...
	if (aw87xxx->open_dsp_en) {
		...
		aw_dev_err(aw87xxx->dev, "%s input nonsupport no_dsp\n",
			__func__);
	}
	mutex_unlock(&aw87xxx->lock);
	return len;
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 5; i++)
		len += snprintf(buf + len, PAGE_SIZE - len,
				"raw_gain_array[%d] = 0x%02X\n", i,
				aw87xxx->raw_gain_array[i]);
...
static DEVICE_ATTR(actflag, S_IWUSR | S_IRUGO,
		aw87xxx_get_actflag, aw87xxx_set_actflag);
static DEVICE_ATTR(aftask, S_IWUSR | S_IRUGO,
		aw87xxx_get_aftask, aw87xxx_set_aftask);
static DEVICE_ATTR(actgain, S_IWUSR | S_IRUGO,
		aw87xxx_get_actgain, aw87xxx_set_actgain);
				    ...
				    struct device_node *np)
{
	...
	aw87xxx->actflag_auto_run = of_property_read_bool(np, "actflag-auto-run");
