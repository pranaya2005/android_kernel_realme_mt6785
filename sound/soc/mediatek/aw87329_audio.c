/*
 * aw87329_audio.c   aw87329 pa module
 *
 * Version: v1.1.3
 *
...
#include "aw87329_audio.h"

/***************************************************
 * aw87329 marco
 ***************************************************/
#define AW87329_I2C_NAME        "aw87329_pa"
#define AW87329_DRIVER_VERSION  "v1.1.3"

/****************************************************
 * aw87329 variable
 ***************************************************/
struct aw87329 *aw87329;
 ...
 * i2c write and read
*********************************************************/
static int aw87329_i2c_write(struct aw87329 *aw87329,
			     unsigned char reg_addr, unsigned char reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw87329->i2c_client,
						reg_addr, reg_data);
		if (ret < 0) {
...
static int aw87329_i2c_read(struct aw87329 *aw87329,
			    unsigned char reg_addr, unsigned char *reg_data)
{
	int ret = 1;
	unsigned char cnt = 0;
...
*******************************************************/
unsigned int aw87329_hw_on(struct aw87329 *aw87329)
{
	pr_info("%s enter\n", __func__);
...
unsigned int aw87329_hw_off(struct aw87329 *aw87329)
{
	pr_info("%s enter\n", __func__);
...
unsigned char aw87329_audio_kspk(void)
{
	...
	if (!aw87329->hwen_flag)
		aw87329_hw_on(aw87329);

	length = sizeof(aw87329_kspk_cfg_default) / sizeof(char);
	/* update default data */
	if (aw87329->kspk_cfg_update_flag == 0) {
		for (i = 0; i < length; i = i + 2) {
			aw87329_i2c_write(aw87329,
					  aw87329_kspk_cfg_default[i],
					  aw87329_kspk_cfg_default[i + 1]);
		}
	}
	/* update firmware data */
	if (aw87329->kspk_cfg_update_flag == 1) {
		for (i = 0; i < aw87329_kspk_cnt->len; i = i + 2) {
			aw87329_i2c_write(aw87329,
					  aw87329_kspk_cnt->data[i],
					  aw87329_kspk_cnt->data[i + 1]);
		}
	}
...
unsigned char aw87329_audio_drcv(void)
{
	...
	length = sizeof(aw87329_drcv_cfg_default) / sizeof(char);
	if (aw87329->drcv_cfg_update_flag == 0) {	/*send default data */
		for (i = 0; i < length; i = i + 2) {
			aw87329_i2c_write(aw87329,
					  aw87329_drcv_cfg_default[i],
					  aw87329_drcv_cfg_default[i + 1]);
		}
	}

	if (aw87329->drcv_cfg_update_flag == 1) {	/*send firmware data */
		for (i = 0; i < aw87329_drcv_cnt->len; i = i + 2) {
			aw87329_i2c_write(aw87329,
					  aw87329_drcv_cnt->data[i],
					  aw87329_drcv_cnt->data[i + 1]);
		}
	}
	return 0;
}
...
unsigned char aw87329_audio_abrcv(void)
{
	...
	length = sizeof(aw87329_abrcv_cfg_default) / sizeof(char);
	if (aw87329->abrcv_cfg_update_flag == 0) {	/*send default data */
		for (i = 0; i < length; i = i + 2) {
			aw87329_i2c_write(aw87329,
					  aw87329_abrcv_cfg_default[i],
					  aw87329_abrcv_cfg_default[i + 1]);
		}
	}

	if (aw87329->abrcv_cfg_update_flag == 1) {	/*send firmware data */
		for (i = 0; i < aw87329_abrcv_cnt->len; i = i + 2) {
			aw87329_i2c_write(aw87329,
					  aw87329_abrcv_cnt->data[i],
					  aw87329_abrcv_cnt->data[i + 1]);
		}
	}
	return 0;
}
...
unsigned char aw87329_audio_off(void)
{
	...
	if (aw87329->hwen_flag)
		aw87329_i2c_write(aw87329, 0x01, 0x00);	/*CHIP Disable */

	aw87329_hw_off(aw87329);
...
static void aw87329_abrcv_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		...
		if (abrcv_load_cont <= 2) {
			schedule_delayed_work(&aw87329->ram_work,
...
static int aw87329_abrcv_update(struct aw87329 *aw87329)
{
	pr_info("%s enter\n", __func__);
	return request_firmware_nowait(THIS_MODULE,
				       FW_ACTION_HOTPLUG,
				       aw87329_abrcv_name,
				       &aw87329->i2c_client->dev,
				       GFP_KERNEL,
				       aw87329, aw87329_abrcv_cfg_loaded);
}
...
static void aw87329_drcv_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (aw87329_drcv_cnt != NULL)
		aw87329_drcv_cnt = NULL;
...
static int aw87329_drcv_update(struct aw87329 *aw87329)
{
	pr_info("%s enter\n", __func__);
	return request_firmware_nowait(THIS_MODULE,
				       FW_ACTION_HOTPLUG,
				       ...
				       &aw87329->i2c_client->dev,
				       GFP_KERNEL,
				       aw87329, aw87329_drcv_cfg_loaded);
}
...
static void aw87329_kspk_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (aw87329_kspk_cnt != NULL)
		aw87329_kspk_cnt = NULL;
...
static int aw87329_kspk_update(struct aw87329 *aw87329)
{
	pr_info("%s enter\n", __func__);
	...
	return request_firmware_nowait(THIS_MODULE,
				       ...
				       &aw87329->i2c_client->dev,
				       GFP_KERNEL,
				       aw87329, aw87329_kspk_cfg_loaded);
}

static void aw87329_cfg_work_routine(struct work_struct *work)
{
	pr_info("%s enter\n", __func__);
	if (aw87329->kspk_cfg_update_flag == 0)
		aw87329_kspk_update(aw87329);
	if (aw87329->drcv_cfg_update_flag == 0)
		aw87329_drcv_update(aw87329);
	if (aw87329->abrcv_cfg_update_flag == 0)
		aw87329_abrcv_update(aw87329);
}
#endif

static int aw87329_cfg_init(struct aw87329 *aw87329)
{
	int ret = -1;
#ifdef AWINIC_CFG_UPDATE_DELAY
	int cfg_timer_val = 5000;

	INIT_DELAYED_WORK(&aw87329->ram_work, aw87329_cfg_work_routine);
	schedule_delayed_work(&aw87329->ram_work,
 ...
 * aw87329 attribute
***********************************************************/
static ssize_t aw87329_get_reg(struct device *cd,
			       struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < aw87329_REG_MAX; i++) {
		if (aw87329_reg_access[i] & REG_RD_ACCESS) {
			aw87329_i2c_read(aw87329, i, &reg_val);
...
static ssize_t aw87329_set_reg(struct device *dev,
			       ...
			       size_t len)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2)
		aw87329_i2c_write(aw87329, databuf[0], databuf[1]);

	return len;
}

static ssize_t aw87329_get_hwen(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static ssize_t aw87329_set_hwen(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t len)
{
	ssize_t ret;
 ...
 out:
	dev_err(&aw87329->i2c_client->dev,
...
static ssize_t aw87329_get_update(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static ssize_t aw87329_set_update(struct device *dev,
...
static ssize_t aw87329_get_mode(struct device *cd,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static ssize_t aw87329_set_mode(struct device *cd,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	ssize_t ret;
...
int aw87329_hw_reset(struct aw87329 *aw87329)
{
	pr_info("%s enter\n", __func__);
 ...
 * check chip id
 *****************************************************/
int aw87329_read_chipid(struct aw87329 *aw87329)
{
	unsigned int cnt = 0;
	int ret = -1;
	unsigned char reg_val = 0;
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		ret = aw87329_i2c_read(aw87329, aw87329_REG_CHIPID, &reg_val);
		if (reg_val == AW87329_CHIPID) {
			pr_info ("%s This Chip is  AW87329 chipid=0x%x\n",
				 __func__, reg_val);
			return 0;
		}
		cnt++;
 ...
 *********************************************************/
static int
aw87329_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	aw87329 = devm_kzalloc(&client->dev,
			       sizeof(struct aw87329), GFP_KERNEL);
	if (aw87329 == NULL) {
		ret = -ENOMEM;
	...
	/* hardware reset */
	aw87329_hw_reset(aw87329);

	/* aw87329 chip id */
	ret = aw87329_read_chipid(aw87329);
	if (ret < 0) {
		dev_err(&client->dev,
			"%s:aw87329_read_chipid failed ret=%d\n",
			__func__, ret);
...
{
	struct aw87329 *aw87329 = i2c_get_clientdata(client);
