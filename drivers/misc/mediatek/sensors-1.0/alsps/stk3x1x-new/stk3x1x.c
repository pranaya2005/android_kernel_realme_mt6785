...
*******************************************************************************/
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	200//400 //200
	#define STK_LT_N_CT	400 //200 //100
	#define STK_HT_N_CT	 600 //300  //150
...
#ifdef STK_TUNE1
	#define STK_FIN_THD			(1500)
	#define STK_ALS_NEAR			(40)
	#define STK_ALS_FAR			(50)
	#define STK_PS_HIGHER_THDH	(STK_FIN_THD - 200)
	#define STK_PS_HIGHER_THDL	(STK_FIN_THD - 300)	

	#define STK_PS_CT_LIMIT			STK_FIN_THD
	#define PS_NOISE					(10)	
	#define STK_CT_UPDATE_DIFF		(20)	
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if(enable)
	{
		...
		APS_LOG("%s: ps enable set HT=%d, LT=%d\n", __func__,
...
/* eint request */
	if (stk3x1x_obj->irq_node) {
		//irq_gpio = of_get_named_gpio(node, "als_int", 0);
		//printk("stk irq_gpio = %d\n", irq_gpio);
		//stk3x1x_obj->irq = gpio_to_irq(irq_gpio); 
		...
		//	ret = gpio_request(irq_gpio, "alsps irq_gpio");
		//	if (ret) {
		//		printk("stk unable to request irq_gpio GPIO %d\n",irq_gpio);
		//	}else
		//		gpio_direction_input (irq_gpio);
		//}else
		//	printk("stk invalid irq_gpio GPIO %d\n",irq_gpio);
...
//	obj->hw = get_cust_alsps_hw();
	...
	//obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, alsps_mtk-eint");
	obj->irq_node = client->dev.of_node;
	atomic_set(&obj->state_val, 0);
	atomic_set(&obj->psctrl_val, 0x33);//0x30
