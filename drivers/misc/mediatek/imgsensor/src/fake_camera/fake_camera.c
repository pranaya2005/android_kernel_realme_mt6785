...
static UINT16 yuv_sensor2_read_shutter(void)
{
  UINT16 light = 0;

  if((NULL != pfFuncs) && (isExist == true))
    light = (*pfFuncs)->getYUVSensor2BV();
  else
    YUV_DBG("get bv fail!\n");
...
void GetYUVSensor2BV(UINT32 *val)
{
    *val = yuv_sensor2_read_shutter();
    YUV_DBG("GetYUVSensor2BV val:%d\n",*val);
}
