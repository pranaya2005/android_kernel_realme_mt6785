...
/* Stoneoim:zhangqingzhan on: Wed, 20 Nov 2019 15:51:14 +0800
 * for camera id export
 */
int g_CameraSensorIdx = 0;
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	IMGSENSOR_PROFILE_INIT(&psensor_inst->profile_time);
/* Stoneoim:zhangqingzhan on: Wed, 20 Nov 2019 15:51:42 +0800
 *for camera id export
 */
		g_CameraSensorIdx = psensor_inst->sensor_idx;
// End of Stoneoim: zhangqingzhan
	ret = imgsensor_hw_power(&pimgsensor->hw,
			psensor,
			IMGSENSOR_HW_POWER_STATUS_ON);
