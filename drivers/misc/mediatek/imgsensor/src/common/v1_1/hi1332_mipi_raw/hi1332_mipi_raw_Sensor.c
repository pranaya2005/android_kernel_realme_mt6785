...
{
    LOG_INF("hi1332. image_mirror = %d", image_mirror);
...
write_cmos_sensor(0x003d, 0x0002);
...
write_cmos_sensor(0x003d, 0x0001);
            ...
            do {
                *sensor_id = ((read_cmos_sensor(0x0F17) << 8) | read_cmos_sensor(0x0F16));
                ...
                if (*sensor_id == imgsensor_info.sensor_id) {
                    LOG_INF("get_imgsensor_id[hi1332] OK. i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                    return ERROR_NONE;
            }
                LOG_INF("get_imgsensor_id[hi1332] Fail. i2c: 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                retry--;
            } while(retry > 0);
            i++;
            retry = 2;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	hs_video_setting();
	set_mirror_flip(imgsensor.mirror);
    //Hynix_i2c_burst(hi1332_hs_video_setting, sizeof(hi1332_hs_video_setting) / 2);  //[mook] [20160217] 640 480 ??????′ù
    return ERROR_NONE;
}    /*    hs_video   */
