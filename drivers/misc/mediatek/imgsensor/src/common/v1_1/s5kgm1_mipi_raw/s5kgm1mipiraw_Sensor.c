...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x6e9db31e, //0x49c09f86,		//checksum value for Camera Auto Test
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  write_cmos_sensor(0x6F12, 0x0100);
  write_cmos_sensor(0x6028, 0x4000);
  write_cmos_sensor(0x0D00, 0x0101);//0100cap
  write_cmos_sensor(0x0D02, 0x0101);//pdaf
  write_cmos_sensor(0x0114, 0x0301);
...
static void normal_video_setting(kal_uint16 currefps)
{
  ...
  write_cmos_sensor(0x6214, 0x7971);
  write_cmos_sensor(0x6218, 0x7150);
  write_cmos_sensor(0x0344, 0x0070);
  write_cmos_sensor(0x0346, 0x0044);
  write_cmos_sensor(0x0348, 0x100F);
  write_cmos_sensor(0x034A, 0x0BFB);
  write_cmos_sensor(0x034C, 0x07D0);
  write_cmos_sensor(0x034E, 0x05DC);
  write_cmos_sensor(0x0340, 0x0CF2);
  write_cmos_sensor(0x0342, 0x12F0);
  ...
  write_cmos_sensor(0x0B06, 0x0101);
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x1FF6);
  write_cmos_sensor(0x6F12, 0x0000);
  write_cmos_sensor(0x021E, 0x0000);
  write_cmos_sensor(0x0202, 0x0100);
  write_cmos_sensor(0x0204, 0x0020);
  write_cmos_sensor(0x0D00, 0x0101);//pdaf video
  write_cmos_sensor(0x0D02, 0x0101);
  write_cmos_sensor(0x0114, 0x0301);
  write_cmos_sensor(0x0D06, 0x0208);
  write_cmos_sensor(0x0D08, 0x0300);
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x0F10);
  write_cmos_sensor(0x6F12, 0x0003);
  write_cmos_sensor(0x602A, 0x0F12);
  write_cmos_sensor(0x6F12, 0x0200);
  write_cmos_sensor(0x602A, 0x2BC0);
  write_cmos_sensor(0x6F12, 0x0001);
  write_cmos_sensor(0x0B30, 0x0000);
  write_cmos_sensor(0x0B32, 0x0000);
  write_cmos_sensor(0x0B34, 0x0001);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    /*if(hbb_flag == 0)
    {
        ...
        while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		    ...
		    //read_imx135_otp_mtk_fmt();
    #endif
				    LOG_INF("s5kgm1 i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
				//s5kgm1kwCheckLensVersion(imgsensor.i2c_write_id);
                    return ERROR_NONE;
                }
			    LOG_INF("s5kgm1 Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
                retry--;
            } while(retry > 0);
            i++;
            retry = 1;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("s5kgm1 open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("s5kgm1 openRead sensor id fail, id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
