...
#include "ov16880jcmipiraw_Sensor.h"

#ifdef CONFIG_FAKE_DUAL_CAMERA
#include "fake_camera.h"
#endif

#define PFX "OV16880JC_camera_sensor"
#define LOG_1 LOG_INF("OV16880JC,MIPI 4LANE,PDAF\n")
...
extern bool jc_read_eeprom( u16 addr, BYTE* data, u32 size);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV16880JC_SENSOR_ID,
	.checksum_value = 0x7804da98,

	.pre = {
		.pclk = 288000000,											//record different mode's pclk
		.linelength = 2512,		/*ov16880jc Note: linelength/4,it means line length per lane*/
	...
	.cap = { /* 95:line 5312, 52/35:line 5336 */
		.pclk = 576000000,
		.linelength = 5024, /*ov16880jc Note: linelength/4,it means line length per lane*/
		.framelength = 3824,
		.startx = 0,
		.starty = 0,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_8(0x3036, 0x20);//zxy
...
{
	/*
	* @@ ov16880jc 4lane 2336x1752 30fps hvbin
	* 100 99 2336 1752
...
//	int retry=0;
	...
	/*
	 * @@ ov16880jc 4lane 4672x3504 30fps
	 * 100 99 4672 3504
...
{
//	int retry=0;
	//LOG_INF( "ov16880jc PIP setting Enter!");

 	/*
	 *@@ ov16880jc 4lane 4672x3504 15fps
	 *100 99 4672 3504
	 *102 3601 bb8 ;Pather tool use only
	 *Pclk clock frequency: 288Mhz
	 *linelength = 5024(0x13a0)
...
static void hs_video_setting(void)
{
	...
	/*
	*@@ ov16880jc 4lane 1280x720 120fps
	*100 99 1280 720
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {	
			*sensor_id = (read_cmos_sensor_8(0x300A) << 16) | (read_cmos_sensor_8(0x300B)<<8) | read_cmos_sensor_8(0x300C) + 2;
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) 
	{
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			jc_read_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
		...
		case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
			...
			//PDAF capacity enable or not, ov16880jc only full size support PDAF
			switch (*feature_data) 
			{
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
...
UINT32 OV16880JCMIPISensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV16880JC_MIPI_RAW_SensorInit	*/
