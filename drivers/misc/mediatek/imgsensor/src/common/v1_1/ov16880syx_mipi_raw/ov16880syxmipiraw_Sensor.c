...
#include "ov16880syxmipiraw_Sensor.h"

#ifdef CONFIG_FAKE_DUAL_CAMERA
#include "fake_camera.h"
#endif

#define PFX "OV16880SYX_camera_sensor"
#define LOG_1 LOG_INF("OV16880SYX,MIPI 4LANE,PDAF\n")
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV16880SYX_SENSOR_ID,
	.checksum_value = 0x7804da98,

	.pre = {
		.pclk = 288000000,											//record different mode's pclk
		.linelength = 2512,		/*OV16880SYX Note: linelength/4,it means line length per lane*/
	...
	.cap = { /* 95:line 5312, 52/35:line 5336 */
		.pclk = 576000000,
		.linelength = 5024, /*OV16880SYX Note: linelength/4,it means line length per lane*/
		.framelength = 3824,
		.startx = 0,
		.starty = 0,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_8(0x3036, 0x6c);//zxy
...
{
	/*
	* @@ OV16880SYX 4lane 2336x1752 30fps hvbin
	* 100 99 2336 1752
...
//	int retry=0;
	...
	/*
	 * @@ OV16880SYX 4lane 4672x3504 30fps
	 * 100 99 4672 3504
...
{
//	int retry=0;
	//LOG_INF( "OV16880SYX PIP setting Enter!");

 	/*
	 *@@ OV16880SYX 4lane 4672x3504 15fps
	 *100 99 4672 3504
	 *102 3601 bb8 ;Pather tool use only
	 *Pclk clock frequency: 288Mhz
	 *linelength = 5024(0x13a0)
...
static void hs_video_setting(void)
{
	...
	/*
	*@@ OV16880SYX 4lane 1280x720 120fps
	*100 99 1280 720
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {	
	                ...
	                mDELAY(20);
                  /*      
	                write_cmos_sensor_8(0x7010, 0x01);
	                ...
	                write_cmos_sensor_8(0x3D8B, 0x10);
	                write_cmos_sensor_8(0x3D80, 0x01);
	                mDELAY(20);
	                write_cmos_sensor_8(0x3D80, 0x00);
                    */
	                write_cmos_sensor_8(0x3D81, 0x01);
	                mDELAY(20);

	                module_info = read_cmos_sensor_8(0x7010);
                        printk("%s module_info = 0x%x\n",__func__,module_info);
			*sensor_id = ((read_cmos_sensor_8(0x300A) << 16) | (read_cmos_sensor_8(0x300B)<<8) | read_cmos_sensor_8(0x300C)) + (module_info == 1 ? 3 : 0);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) 
	{
		...
		case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
			...
			//PDAF capacity enable or not, OV16880SYX only full size support PDAF
			switch (*feature_data) 
			{
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        ...
        case SENSOR_FEATURE_GET_YUV_SENSOR_BV:
            ...
            break;
        case SENSOR_FEATURE_GET_YUV_SENSOR2_BV:
            printk("SENSOR_FEATURE_GET_YUV_SENSOR_BV\n");
            GetYUVSensor2BV(feature_data_32);
            break;
...
UINT32 OV16880SYXMIPISensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV16880SYX_MIPI_RAW_SensorInit	*/
