...
extern int hsm_iWriteRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u16 i2cId);
...
extern int hsm_g_cmd_run(void);
extern int hsm_s_cmd_run(void);
extern int hsm_dumg_reg(void);
extern int hsm_iReadRegI2C_MIPI(u16 addr , u8 * rxbuf, u16 len, u16 i2cId);
extern int honeywell_bridge_read_bytes_non_dma(char *writebuf, int writelen, char *readbuf, int readlen);
extern int hsm_iReadRegI2C_ext(char *writebuf, int writelen, char *readbuf, int readlen);
           ...
           hsm_dbg("[hsm_ioctl] HSM_IIC_READ\n");        
            {
                ...
                *len = 0;
                ...
                if( iic->len <= sizeof(buf) ) 
                {
					...
					if((iic->i2c_addr ==MT9M114_I2C_ADDR) || ((engine_id == HSM_ENGINE_N6700) && (iic->i2c_addr ==AR0144_I2C_ADDR)) )
                ...
                *len = 0;
                ...
                { 
            		...
            		if(iic->i2c_addr ==PSOC_I2C_ADDR || iic->i2c_addr == 0x10 || iic->i2c_addr ==AIMER_I2C_ADDR || iic->i2c_addr ==ILLUM_I2C_ADDR || ((engine_id == HSM_ENGINE_N5600)&& (iic->i2c_addr ==EV76C454_I2C_ADDR))){
						...
						buf[0]= iic->reg;
                    err = iWriteRegI2CTiming(buf, iic->len + 1, iic->i2c_addr << 1,100);
            			}
					if(iic->i2c_addr ==TC358748_I2C_ADDR){
...
static kal_uint16 N6700_read_cmos_sensor(kal_uint16 addr)
{
	...
	hsm_dbg("reg is %x, addr is %x,read value is %x\n",addr,N6700_WRITE_ID,in_buff); 
	if(ret < 0)
    {
    	hsm_dbg("in_buf = %d\n", in_buff);
...
static kal_uint16 N5600_read_cmos_sensor(kal_uint8 addr)
{
    ...
    kal_uint8   out_buff = addr;
     SENSORDB("%s enter\n",__FUNCTION__); 
    if (0 != iReadRegI2CTiming((u8*)&out_buff , (u16) sizeof(out_buff), (u8*)&in_buff, (u16) sizeof(in_buff), N5600_WRITE_ID,100)) 
    {
...
static kal_uint32 HsmOpen(void)
{
   ...
   // kal_uint16 tempid=0; 
	...
	else if(engine_id == HSM_ENGINE_N5600)
	{
	...
	else if(engine_id == HSM_ENGINE_N3600)
	{
	...
	else if(engine_id == HSM_ENGINE_N3603)
	{
		hsm_props.width = N3600_IMAGE_SENSOR_WIDTH;//
        hsm_props.height = N3600_IMAGE_SENSOR_HEIGHT;//
		hsm_props.i2c_addr_sensor = MT9M114_I2C_ADDR;
...
static kal_uint32 Hsm_GetSensorID(kal_uint32 *sensorID)
{
    ...
    msleep(120);
	tempid = N6700_read_cmos_sensor(AR0144_ID_REG);
	hsm_dbg("N6700 Sensor ID = 0x%x\n", tempid);
	if(tempid == AR0144_REVISION)
	{
		engine_id = HSM_ENGINE_N6700;
		hsm_dbg("found N6700 Engine\n");
	}
	else
	{
		tempid = N5600_read_cmos_sensor(JADE_ID_REG);
		hsm_dbg("N5600 sensor ID = 0x%x\n", tempid);
		if(tempid == JADE_REVISION || tempid == JADE_REVISION_2)
		{
			engine_id = HSM_ENGINE_N5600;
			hsm_dbg("found N5600 Engine\n");
		}
		else
		{
			tempid = N3600_read_cmos_sensor(MT9M114_CHIP_VERSION_REG);
			hsm_dbg("N360X sensor ID = 0x%x\n", tempid);
			if(tempid == MT9M114_REVISION)
			{
				int err=0;
				u8 data[4];
                memset(data, 0, 4);
				err =hsm_iReadRegI2C(0x0, data, 4, 0x80);
				hsm_dbg(" POSC check Engine ID = %x%x%x%x\n",data[0],data[1],data[2],data[3]);
				if(data[2] == 0x6E)
				{
					engine_id = HSM_ENGINE_N3603;
					hsm_dbg("found N3603 Engine\n");
				}
				else
				{
					engine_id = HSM_ENGINE_N3600;
					hsm_dbg("found N3601 Engine\n");
				}
			}
...

{
	...
	else if((engine_id == HSM_ENGINE_N3600) || (engine_id == HSM_ENGINE_N3603))
	{
        ...
        MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	if(hsm_props.Reserved[0] == HSM_MIPI_1_LANE)
    pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_1_LANE;
	else if(hsm_props.Reserved[0] == HSM_MIPI_2_LANE)
...
static kal_uint32 HsmFeatureControl(MSDK_SENSOR_FEATURE_ENUM id, kal_uint8 *para, kal_uint32 *len)
{
    ...
    switch (id)
    {
        ...
        case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
            ...
            break;
        default:
			hsm_dbg("sunyasheng [hsm_ioctl] HsmFeatureControl id %d\n",id);
