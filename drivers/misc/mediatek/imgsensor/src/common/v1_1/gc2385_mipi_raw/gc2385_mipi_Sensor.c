...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x7a06585a, //0xf7375923,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0, 0, 1600, 1200} // custom1  // add for dual camera
};
		...
		do {
			*sensor_id = return_sensor_id() & 0xffef;
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("gc2385 i2c write id: 0x%x, sensor id: 0x%x,sensor_id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id,imgsensor_info.sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("gc2385 Read sensor id fail, write id: 0x%x, id: 0x%x,sensor_id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id,imgsensor_info.sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() & 0xffef;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("gc2385 open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			LOG_INF("gc2385 open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
