...
#define PFX "OV8865"
#define LOG_1 LOG_INF("OV8865,MIPI CAM\n")
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x3006,0x00);//wuxiaotian ov8865 address:0x3006  default value:0x42  gc0310&gc030a conflicting
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
