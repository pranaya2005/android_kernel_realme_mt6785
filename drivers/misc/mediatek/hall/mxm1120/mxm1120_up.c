...
static struct i2c_client *mxm1120_up_i2c_client;
/***********************************************************/
...
#if 1
void m1120_up_enable(int enable)
{
  if (enable) {                  /*enable if state will be changed*/
    m1120_i2c_set_reg(mxm1120_up_i2c_client, M1120_REG_INTSRS, 0x80);
    m1120_i2c_set_reg(mxm1120_up_i2c_client, M1120_REG_OPF, 0x41);
  } else {                        /*disable if state will be changed*/
    m1120_i2c_set_reg(mxm1120_up_i2c_client, M1120_REG_OPF, 0x30);
  }
}
#endif
static void m1120_set_enable(struct device *dev, int enable)
...
#if 1
int m1120_up_measure(short *raw)
{
    int err = 0;
    int up_retry = 3;
    u8 buf[3];
    ...
    while(up_retry)
    {
      err = m1120_i2c_read(mxm1120_up_i2c_client, M1120_REG_ST1, buf, sizeof(buf));
      if (err >= 0)
        break;
      up_retry--;
      m1120_up_enable(0);
      ...
      msleep(50);
        ...
        *raw = m1120_convdata_2byte_to_short(0, buf[2], buf[1]);
    if (1) {
        printk("raw data (%d)\n", *raw);
    }
...
static long m1120_misc_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case M1120_IOCTL_SET_ENABLE:
        ret = copy_from_user((void *)&kbuf, (void *)argp, sizeof(kbuf));
        if (ret)
            return -EFAULT;
    ...
    case M1120_IOCTL_SET_CALIBRATION:
        ...
        ret = copy_to_user((void *)argp, (void *)&kbuf, sizeof(kbuf));
        if (ret)
            return -EFAULT;
        break;
    case M1120_IOCTL_GET_CALIBRATED_DATA:
        ...
        ret = copy_to_user((void *)argp, (void *)&caldata, sizeof(caldata));
        if (ret)
            return -EFAULT;
...
static int m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    err = sysfs_create_group(&p_data->client->dev.kobj, &m1120_attribute_group);
    if (err) {
