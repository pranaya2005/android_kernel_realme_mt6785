...
static struct i2c_client *mxm1120_down_i2c_client;
/***********************************************************/
...
#if 1
void m1120_down_enable(int enable)
{
  if (enable) {                  /*enable if state will be changed*/
    m1120_i2c_set_reg(mxm1120_down_i2c_client, M1120_REG_INTSRS, 0x80);
    m1120_i2c_set_reg(mxm1120_down_i2c_client, M1120_REG_OPF, 0x41);
  } else {                        /*disable if state will be changed*/
    m1120_i2c_set_reg(mxm1120_down_i2c_client, M1120_REG_OPF, 0x30);
  }
}
#endif
static void m1120_set_enable(struct device *dev, int enable)
...
#if 1
int m1120_down_measure(short *raw)
{
    int err = 0;
    int retry=3;
    u8 buf[3];
    ...
    while(retry)
    {
      err = m1120_i2c_read(mxm1120_down_i2c_client, M1120_REG_ST1, buf, sizeof(buf));
      if (err >= 0)
        break;
        retry--;
        m1120_down_enable(0);
...
static int m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    mxm1120_down_i2c_client=client;
