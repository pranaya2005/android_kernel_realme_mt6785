...
static int adc_read_vol(void)
{
  ...
  adc_vol = sum/10;
#if defined(BUILD_LK)
  printf("ili9882h adc_vol is %d\n",adc_vol);
#else
  printk("ili9882h adc_vol is %d\n",adc_vol);
#endif
...
#if defined(BUILD_LK)
	printf("%s,ili9882h_panda652_haifei_hdplus1600 id = 0x%08x\n", __func__, id);
#else
	printk("%s,ili9882h_panda652_haifei_hdplus1600 id = 0x%08x\n", __func__, id);
#endif
	return (id == 0x82 )?1:0;
}

struct LCM_DRIVER ili9882h_panda652_haifei_hdplus1600_lcm_drv = {
	.name 				= "ili9882h_panda652_haifei_hdplus1600",
