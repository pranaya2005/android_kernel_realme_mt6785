...
/* Foeec:zhangqingzhan on: Sat, 11 Jan 2020 18:18:11 +0800
 * for camera moto
 */
#ifdef CONFIG_IMGSENSOR_MOTO
void report_home_key_press(void)
{
			input_report_key(kpd_input_dev, KEY_HOMEPAGE, 1);
			input_sync(kpd_input_dev);
}
void report_home_key_up(void)
{
			input_report_key(kpd_input_dev, KEY_HOMEPAGE, 0);
			input_sync(kpd_input_dev);
}
#endif
...
#endif
/* Foeec:zhangqingzhan on: Sat, 11 Jan 2020 18:19:10 +0800
 * for camera moto
 */
#ifdef CONFIG_IMGSENSOR_MOTO
		__set_bit(KEY_HOMEPAGE, kpd_input_dev->keybit);
#endif
// End of Foeec: zhangqingzhan
