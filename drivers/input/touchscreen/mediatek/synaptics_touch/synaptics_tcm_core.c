...
static ssize_t syna_tcm_sysfs_fwid_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	struct device *p_dev;
	...
	retval = snprintf(buf, PAGE_SIZE,
			"device_fw_id %d,config_id %8.8s\n",
			tcm_hcd->packrat_number,tcm_hcd->app_info.customer_config_id);
	if (retval < 0)
		goto exit;
exit:
...
extern void lcm_set_reset_pin(bool bEnable);
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	 ...
	 */
	{
		...
		v_set_dev_name(2, "synaptics_tcm");
	}
