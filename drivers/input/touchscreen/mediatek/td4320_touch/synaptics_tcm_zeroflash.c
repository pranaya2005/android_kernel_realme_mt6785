...
//extern void mt_set_spi_clk_3M(void);
//extern void mt_set_spi_clk_500K(void);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	const struct syna_tcm_board_data *bdata = zeroflash_hcd->tcm_hcd->hw_if->bdata;
	...
	if(tcm_hcd->host_download_mode) {
		...
		if(fcounts != 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
				"Failed %u times for host download in %u test cycles\n", fcounts, times);
		} else {
			LOGD(tcm_hcd->pdev->dev.parent,
...
static int zeroflash_download_app_fw(void)
{
	...
	out_buf_size = (((image_info->app_firmware.size + 1023) >> 10) << 10) - 2;
	LOGE(tcm_hcd->pdev->dev.parent,
