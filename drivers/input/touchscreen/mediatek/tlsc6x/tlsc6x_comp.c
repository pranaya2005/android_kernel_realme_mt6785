...
static u32 fw_size = ARRAY_SIZE(fw_file_tlsc6x);
// Telink CTP
...
int tlsc6x_download_ramcode(u8* pcode, u16 len)
{
	...
	msleep(30);   // let caller decide the delay time ?
...
{
	if (ptcfg == NULL) {
		printk("is_valid_cfg_data fail \n");
		return 0;
	}
   TLSC_FUNC_ENTER();
   printk("ptcfg address:g_tlsc6x_client->addr:%d:%d\n",(ptcfg[53]>>1)&0x7f,g_tlsc6x_client->addr); 
    if((u8)((ptcfg[53]>>1)&0x7f) != (u8)(g_tlsc6x_client->addr)){
	printk("ptcfg is fail \n");
        return 0;
    }

    if(tlsc6x_checksum_u16(ptcfg, 204)){
	printk("tlsc6x_checksum_u16 fail\n");
        return 0;
    }
...
static int tlsc6x_tpcfg_ver_comp(unsigned short * ptcfg)
{
	...
	printk("g_tlsc6x_cfg_ver=:%d\n",g_tlsc6x_cfg_ver);	
       	return 0;
	}

	if (is_valid_cfg_data(ptcfg) == 0) {
		printk("fail is_valid_cfg_data\n");
		return 0;
	}
...
int tlsc6x_update_compat_ctl(u8 *pupd, int len)
{
	...
	struct tlsc6x_updfile_header *upd_header;
        TLSC_FUNC_ENTER();	
	printk("entry:tlsc6x_update_compat_ctl:len=%d\n",len);
