...
static int core_i2c_write(void *buf, int len)
{
	...
	struct i2c_msg msgs[] = {
		{
		 .addr = idev->i2c->addr,
/* Stoneoim:wuxiaotian on: Wed, 04 Mar 2020 15:43:40 +0800
...
#if I2C_DMA_TRANSFER
	if (len > 8) {
		msgs[0].addr = (idev->i2c->addr & I2C_MASK_FLAG);
		msgs[0].ext_flag = (idev->i2c->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG);
...
static int core_i2c_read(void *buf, int len)
{
	...
	struct i2c_msg msgs[] = {
		{
		 .addr = idev->i2c->addr,
/* Stoneoim:wuxiaotian on: Wed, 04 Mar 2020 15:43:43 +0800
 * TODO: replace this line with your comment
         .timing = 100,
 */
