...
static ssize_t ilitek_proc_fw_version_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	int len = 0;
	...
	len = snprintf(g_user_buf, PAGE_SIZE, "fw_version = 0x%X\n", idev->chip->fw_ver);
	mutex_unlock(&idev->touch_mutex);
...
struct file_operations proc_fw_version_fops = {
	.read = ilitek_proc_fw_version_read,
};
...
proc_node_t proc_table[] = {
	...
	{"fw_version", NULL, &proc_fw_version_fops, false},
