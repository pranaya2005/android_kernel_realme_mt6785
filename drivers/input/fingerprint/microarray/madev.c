...
static unsigned int ma_drv_ver = 0x0000406;
...
static int read_chipid(void) {
	...
	char outid[4] = {0};
	mt_spi_enable_master_clk(smas->spi);
printk("%s: read_chipid  reset=0x%x,0x%x,0x%x,0x%x, start!!\n", __func__, reset[0], reset[1], reset[2], reset[3]);
printk("%s: read_chipid  readid=0x%x,0x%x,0x%x,0x%x, start!!\n", __func__, readid[0], readid[1], readid[2], readid[3]);
printk("%s: read_chipid  outid=0x%x,0x%x,0x%x,0x%x, start!!\n", __func__, outid[0], outid[1], outid[2], outid[3]);
...
printk("%s: read_chipid  outid=0x%x,0x%x,0x%x,0x%x, start!!\n", __func__, outid[0], outid[1], outid[2], outid[3]);
	...
	if (outid[3] == 0x41|| outid[3] == 0x45) {
		printk("[MAFP] chip is not ok\n");
		return 0;
	}
        //close diasble master clk for spi TP bug
	//mt_spi_disable_master_clk(smas->spi);
printk("%s: [MAFP] chip is  not ok\n", __func__);
	return -1;
}
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	case DISABLE_IRQ:
	    ...
	    break;
        case ENABLE_CLK:
            mt_spi_enable_master_clk(smas->spi);                                    //if the spi clock is not opening always, do this methods
            break;
        case DISABLE_CLK:
            mt_spi_disable_master_clk(smas->spi);                                 //disable the spi clock
            break;
        case ENABLE_INTERRUPT:
            enable_irq(irq);                                                    //enable the irq,in fact, you can make irq enable always
            break;
        case DISABLE_INTERRUPT:
            disable_irq_nosync(irq);                                                    //disable the irq
            break;
        case TAP_DOWN:
        ...
        case MA_IOC_GVER:
            mutex_lock(&ioctl_lock);
            ret = copy_to_user((unsigned int*)arg, &ma_drv_ver, sizeof(unsigned int));
            mutex_unlock(&ioctl_lock);
            break;
		case SCREEN_ON:
			mas_switch_power(1);
...
#ifdef DOUBLE_EDGE_IRQ
	ret = request_irq(irq, mas_interrupt, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT, tname, NULL);
#else
...
#ifdef VANZO_FP_NAME_SUPPORT
    if (ret == 0x41){
    v_set_dev_name(7, "microarray_112n");
    }else if( ret == 0x45){
    v_set_dev_name(7, "microarray_64n");
    }else{
    v_set_dev_name(7, "microarray");
    }
#endif    
    mas_set_input();
    MALOGF("end");
