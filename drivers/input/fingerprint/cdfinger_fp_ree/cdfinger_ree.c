...
#define CDFINGER_DBG(fmt, args...)                                                      \
	...
	{                                                                                   \
		if (cdfinger_debug & 0x01)                                                      \
			printk("chenls [DBG][cdfinger]:%5d: <%s>" fmt, __LINE__, __func__, ##args); \
	} while (0)
...
static int cdfinger_parse_dts(struct cdfinger_data *cdfinger)
{
	...
	{
		cdfinger->cdfinger_spi_miso = pinctrl_lookup_state(cdfinger->fps_pinctrl, "finger_spi_miso");
		...
		cdfinger->cdfinger_spi_mosi = pinctrl_lookup_state(cdfinger->fps_pinctrl, "finger_spi_mosi");
		...
		cdfinger->cdfinger_spi_sck = pinctrl_lookup_state(cdfinger->fps_pinctrl, "finger_spi_sck");
		...
		cdfinger->cdfinger_spi_cs = pinctrl_lookup_state(cdfinger->fps_pinctrl, "finger_spi_cs");
	...
	{
		cdfinger->fps_vio_on = pinctrl_lookup_state(cdfinger->fps_pinctrl, "fp_vio_high");
		...
		cdfinger->fps_vio_off = pinctrl_lookup_state(cdfinger->fps_pinctrl, "fp_vio_low");
...
static int cdfinger_power_on(struct cdfinger_data *cdfinger)
{
	...
	if (cdfinger->vdd_ldo_enable == 1)
	{
		...
		//gpio_direction_output(cdfinger->pwr_gpio, 1);
	}
...
static int cdfinger_probe(struct spi_device *spi)
{
	...
	int checkid_count;
	//CDFINGER_DBG("enter\n");
	printk("chenls cdfinger cdfinger_probe enter\n");
...
#endif
	printk("chenls cdfinger cdfinger_spi_init enter");
