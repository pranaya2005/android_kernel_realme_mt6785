        ...
        char* buffer)
{
    ...
    if (irq_node) {
        ...
        pr_debug("######### %s line %d gpio %d irq_state %d\n",__func__,__LINE__, ints[0], irq_state);
    }
...
/**
 * writing to the irq node will just drop a pr_debug message
...
static void fpc_work(struct work_struct *pws) {
    ...
    if (fpc_wdata->wakeup_enabled) {
        __pm_wakeup_event(&fpc_wdata->ttw_wl,
...
static int fpc1020_irq_function(void *pData)
{
    ...
    pr_debug("fpc, andy hong, fpc1020_irq_function, entry\n");

    while (!kthread_should_stop()) {
        pr_debug("fpc, andy hong, fpc1020_irq_function, wait_event_interruptible\n");
        wait_event_interruptible(fpc1020_irq_return,
                (fpc1020_irq_occur != 0));
        pr_debug("fpc, andy hong, fpc1020_irq_function, sysfs_notify\n");
        fpc1020_irq_occur = 0;
...
static int fpc1020_irq_init(struct fpc1020_data *fpc1020)
{
    ...
    pr_debug(KERN_INFO "fpc, andy hong, %s\n", __func__);
    ...
    if (IS_ERR(fpc1020_irq_thread)) {
        pr_debug("fpc, andy hong, fpc1020_irq_thread, kthread_run failed.\n");
...
static int fpc1020_probe(struct platform_device *pldev)
{
    ...
    //fpc, andy hong, for ree porting, crash at enroll, 20161122
#if 1
    pr_debug(KERN_INFO "fpc, andy hong, %s, fpc1020_irq_init\n", __func__);
    fpc1020_irq_init(fpc1020);
#endif
    //end, fpc, andy hong, for ree porting, crash at enroll, 20161122

    pr_debug("fiona test 002.\n" );
    ...
    if (!workq) {
        pr_debug("%s: create_single_workqueue failed\n", __func__);
        return -ENOMEM;
    }
...
exit:
    ...
    if(rc)
    {
        pr_debug("fpc, andy hong, fpc1020_probe, fpc1020_irq_destroy\n");
...
int fpc1020_get_irqNum(void)
{
    ...
    //irq_node = of_find_compatible_node(NULL, NULL, "mediatek, fpc_finger");
    irq_node = of_find_matching_node(irq_node, fpc1020_of_match);
    if (irq_node) {
        of_property_read_u32_array(irq_node, "interrupts", ints, ARRAY_SIZE(ints));
        gpiopin = ints[0];
        debounce = ints[3];
        ...
        pr_debug("[fpc]%s can't find compatible irq_node\n", __func__);
...
static int fpc1020_remove(struct platform_device *pldev)
{
    ...
    //fpc, andy hong, for ree porting, crash at enroll, 20161122
#if 1
    pr_debug(KERN_INFO "fpc, andy hong, %s, fpc1020_irq_destroy\n", __func__);
    fpc1020_irq_destroy(fpc1020);
#endif
    //end, fpc, andy hong, for ree porting, crash at enroll, 20161122
