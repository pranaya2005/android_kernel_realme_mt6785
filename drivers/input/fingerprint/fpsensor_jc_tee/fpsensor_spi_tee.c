...
#if defined(CHIPONE_TKCORE_COMPATIBLE)
...
static struct TEEC_UUID chipone_vendor_uuid = { 0x7778c03f, 0xc30c, 0x4dd0,
	{0xa3, 0x19, 0xea, 0x29, 0x64, 0x3d, 0x4d, 0x4c }};
#endif
...
/* global variables */
unsigned int fingerprint_compatible=0;
...
static int fpsensor_irq_gpio_cfg(fpsensor_data_t *fpsensor)
{
    ...
    //if ( node) 
    {
        ...
        fpsensor->irq = irq_of_parse_and_map(fpsensor->spi->dev.of_node, 0);  // get irq number
        if (!fpsensor->irq) {
...
#if 0
// There is not mt_chip_conf strunt in MTK6739.
...
/*	
	...
	if (node) {
		...
		if (pdev) 
		{
		status = irq_of_parse_and_map(node, 0);	
		fpsensor_dev->pinctrl1 = devm_pinctrl_get(&pdev->dev);
		
			if (IS_ERR(fpsensor_dev->pinctrl1)) {
				status = PTR_ERR(fpsensor_dev->pinctrl1);
				fpsensor_debug(ERR_LOG,"XXXXXXXXXXX can't find fingerprint pctrl\n");
				return status;
			}
		}else {
			fpsensor_debug(ERR_LOG, " XXXXXXXXX  platform device is null\n");
		}
	}
	else {
		fpsensor_debug(ERR_LOG, " ZZZZZZZZZZZ  device node is null\n");
	}
*/

	fpsensor_debug(ERR_LOG, "fpsensor ZZZZZZZZZZZZZZ.\n");

    spi->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,fpsensor");
    ...
    for(testCnt = 0;testCnt < 5;testCnt++){
        ...
        if (status != 0) {
            udelay(3000);
                ...
                fpsensor_debug(ERR_LOG,"failed to write spi status=%d\n",status);
                goto release_drv_data;
        ...
        if (status == 0) {
            fpsensor_debug(DEBUG_LOG, "chipid_tx : %x  %x  %x  %x \n",
                chipid_tx[0],chipid_tx[1],chipid_tx[2],chipid_tx[3]);
            fpsensor_debug(DEBUG_LOG, "chipid_rx : %x  %x  %x  %x \n",
                chipid_rx[0],chipid_rx[1],chipid_rx[2],chipid_rx[3]);
            hwid = ((chipid_rx[1] << 8) | (chipid_rx[2]));
            if ((hwid == 0x7332) || (hwid == 0x7153) || (hwid == 0x7230) ||(hwid == 0x7222)){
				  fingerprint_compatible= 1;
                fpsensor_debug(DEBUG_LOG,"get HWID == 0x%x .\n",hwid);
                break;
            } 
            else {
				 fingerprint_compatible= 0;
                fpsensor_debug(ERR_LOG,"get id  count = %d\n",testCnt);
                udelay(3000);
                
                    fpsensor_debug(ERR_LOG,"failed to get hwid , status=%d\n",status);
                    goto release_drv_data;
                
            }
        } 
        else {
                fpsensor_debug(ERR_LOG,"failed to callback spi function of tkcore, and failed to get hwid \n");   
